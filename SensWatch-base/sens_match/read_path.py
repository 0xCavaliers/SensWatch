...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(os.path.join(SCRIPT_DIR, "simple_everything.log")),  # 输出到文件
...
# 新增：临时/隐藏文件过滤函数
def is_temp_or_hidden_file(file_path):
        ...
        with self.lock:
            try:
                with open(self.log_file, "w", encoding="utf-8") as f:
                    json.dump(self.directory_log, f, ensure_ascii=False, indent=2)
...
class FileChangeHandler(FileSystemEventHandler):  
    ...
    def on_created(self, event):
        if not event.is_directory and not is_temp_or_hidden_file(event.src_path):
    ...
    def on_moved(self, event):
        if not event.is_directory and not is_temp_or_hidden_file(event.dest_path):
